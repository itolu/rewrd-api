...
export class MerchantService {
     ...
     */
    async getMerchantConfig(merchant_id: string) {
        logger.debug("Fetching merchant configuration", { merchant_id });
        ...
        logger.debug("Merchant configuration retrieved", { merchant_id });
        return merchant;
    }
    ...
    async updateMerchantConfig(merchant_id: string, updates: {
        ...
        if (!merchant) {
            logger.warn("Cannot update - merchant not found", { merchant_id });
            throw new AppError("Merchant not found", 404, "merchant_not_found");
        }
    ...
    async getAnalytics(merchant_id: string, period: string = 'all') {
        ...
        // Points Ledger Aggregation
        const ledgerQuery = db("Pointsledger")
            .where({ merchant_id });
        ...
        const totalBalance = await db("Customers")
            .where({ merchant_id })
            .sum("points_balance as total")
            .first();
